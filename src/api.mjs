...
async function body(req) {
  ...
  for await (const chunk of req) {
    data += chunk;
    if (data.length > 100000) fail(413, "Dados muito grandes.");
  }
  try {
...
async function route(req, res, url, b) {
  ...
  if (match) {
    ...
    if (action === "public" && m === "GET") {
      const e =
        (await get(
          "SELECT id,titulo,tipo,data,local,prazo FROM events WHERE id=?",
          eid,
    ...
    if (action === "response" && m === "POST") {
      ...
      await transaction(async () => {
        await run(
          "UPDATE guests SET status=?,lugares=?,criancas=?,restricao=?,recado=?,respondido=? WHERE id=?",
          b.status,
          lugares,
          criancas,
          restricao,
    ...
    if (!action && m === "PATCH") {
      ...
      await run(
        "UPDATE events SET titulo=?,local=?,data=?,prazo=? WHERE id=?",
        titulo,
        local,
        data,
        prazo,
