...
function guestRow(g, actions = "") {
  return `<td><div class="guest-name">${escapeHTML(g.nome)}</div><div class="guest-sub">${escapeHTML(g.telefone)}</div></td><td><span class="badge ${g.status}">${STATUS_LABEL[g.status]}</span></td><td>${g.lugares || "—"}</td><td>${escapeHTML(g.restricao || "—")}</td><td>${g.criancas || "—"}</td><td>${dateLabel(g.respondido)}</td>${actions}`;
}
